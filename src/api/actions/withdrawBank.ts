...
export default async function withdrawBank(
  character: string,
  values: {
    code: string;
    quantity: number;
  ...
  try {
    const response = await api.post<Data>("/action/bank/withdraw", body);
    ...
    if (err instanceof AxiosError) {
      ...
      console.error(
        `Error Bank Withdraw API: ${character} ${errorKey}` + " " + statusCode
      );
    } else {
